    ...
    return (
        ...
        <div id="aboutus-section">
            <div className='mx-auto max-w-7xl px-4 py-24 my-32 lg:px-10 bg-lightgrey rounded-3xl relative'>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 my-16 gap-x-16 lg:gap-x-32'>
                    {Aboutdata.map((item, i) => (
                        <div key={i} className='hover:bg-navyblue bg-white rounded-3xl mt-16 pt-10 pl-8 pb-10 pr-6 shadow-xl group mb-20'>
                            <h4 className='text-4xl font-semibold  text-blue mb-5 group-hover:text-white'>{item.heading}</h4>
                            {/* <Image src={item.imgSrc} alt={item.imgSrc} width={100} height={100} className="mb-5" /> */}
                            <p className='text-lg font-normal text-blue group-hover:text-offwhite mb-5'>{item.paragraph}</p>
                ...
                <h3 className='text-center text-blue text-4xl tracking-widest'>Níveis</h3>
                <p className='text-center text-2xl lg:text-24xl  text-blue'>O Plano Curricular segue os níveis de referência do Quadro Europeu Comum de Referência para as Línguas (QECR):</p>
                <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 my-16 gap-x-16 lg:gap-x-32'>
                    {Levels.map((item, i) => (
                        <div key={i} className='hover:bg-navyblue bg-white rounded-3xl mt-10 pt-10 pl-8 pb-10 pr-6 shadow-xl group'>
                            <h4 className='text-3xl font-semibold  text-blue mb-5 group-hover:text-white'>{item.heading}</h4>
                            {/* <Image src={item.imgSrc} alt={item.imgSrc} width={100} height={100} className="mb-5" /> */}
                            <p className='text-lg font-normal text-blue group-hover:text-offwhite mb-5'>{item.paragraph}</p>
                            {/* <Link href="#" className='text-lg font-semibold group-hover:text-white text-blue hover-underline'>
                                {item.link}
